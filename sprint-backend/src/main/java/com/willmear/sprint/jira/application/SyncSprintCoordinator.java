...
public class SyncSprintCoordinator {
    ...
    private static final String SPRINT_SYNC_SCOPE_TYPE = "SPRINT";
    ...
    public SyncSprintResult sync(UUID workspaceId, UUID connectionId, Long sprintId, SyncSprintRequest request) {
        ...
        try (LoggingContextHelper.Scope ignored = loggingContextHelper.putAll(Map.of(
            ...
            String sprintScopeReference = String.valueOf(sprintId);
            jiraRawPayloadRepositoryPort.deleteBySyncScope(
                    workspaceId,
                    connectionId,
                    SPRINT_SYNC_SCOPE_TYPE,
                    sprintScopeReference
            );

            ExternalJiraSprintDto sprintDto = jiraClientPort.fetchSprint(connection, sprintId);
            saveRawPayload(workspaceId, connectionId, "SPRINT", String.valueOf(sprintId), sprintScopeReference, sprintDto, syncedAt);
            ...
            if (boardId != null) {
                ...
                saveRawPayload(workspaceId, connectionId, "BOARD", String.valueOf(boardId), sprintScopeReference, boardDto, syncedAt);
            ...
            for (ExternalJiraIssueDto issueDto : issueDtos) {
                saveRawPayload(workspaceId, connectionId, "ISSUE", issueDto.key(), sprintScopeReference, issueDto, syncedAt);
            }
            ...
            for (JiraIssue issue : issues) {
                ...
                if (includeComments) {
                    ...
                    for (ExternalJiraCommentDto commentDto : commentDtos) {
                        saveRawPayload(workspaceId, connectionId, "COMMENT", commentDto.id(), sprintScopeReference, commentDto, syncedAt);
                    }
                }
                ...
                if (includeChangelog) {
                    ...
                    for (ExternalJiraChangelogDto changelogDto : changelogDtos) {
                        saveRawPayload(workspaceId, connectionId, "CHANGELOG", changelogDto.historyId(), sprintScopeReference, changelogDto, syncedAt);
                    }
                }
            }
    ...
    private void saveRawPayload(
            UUID workspaceId,
            UUID connectionId,
            String payloadType,
            String externalReference,
            String sprintScopeReference,
            Object payload,
            Instant syncedAt
    ) {
        com.fasterxml.jackson.databind.JsonNode payloadJson;
        try {
            payloadJson = objectMapper.readTree(objectMapper.writeValueAsBytes(payload));
        } catch (Exception exception) {
            throw new JiraSyncException("Failed to serialize raw Jira payload for type " + payloadType + ".", exception);
        }

...
public class PresentationController {
    ...
    public ResponseEntity<PresentationDeckResponse> createOrGetDeck(
            @PathVariable UUID workspaceId,
            @PathVariable Long sprintId,
            @RequestParam(required = false) String themeId
    ) {
        return ResponseEntity.status(HttpStatus.CREATED).body(
                presentationDeckMapper.toResponse(presentationDeckService.createOrGetDeck(workspaceId, sprintId, themeId))
        );
    }

...
public class SpeakerNotesParser {
    ...
    public SpeakerNotesParser(ObjectMapper objectMapper, AiJsonContentExtractor aiJsonContentExtractor) {
    ...
    public List<SpeakerNote> parse(AiResponse response) {
        try {
            return objectMapper.readValue(aiJsonContentExtractor.extractJsonArray(response.content()), new TypeReference<List<SpeakerNote>>() {
            });

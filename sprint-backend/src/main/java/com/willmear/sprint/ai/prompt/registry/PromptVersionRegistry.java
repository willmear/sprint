package com.willmear.sprint.ai.prompt.registry;

public final class PromptVersionRegistry {

    public static final String SPRINT_REVIEW_VERSION = "v2";
    public static final String SPRINT_SUMMARY_VERSION = "v1";
    public static final String THEME_EXTRACTION_VERSION = "v1";
    public static final String SPRINT_HIGHLIGHTS_VERSION = "v1";
    public static final String SPRINT_BLOCKERS_VERSION = "v1";
    public static final String SPEAKER_NOTES_VERSION = "v1";

    private PromptVersionRegistry() {
    }
}

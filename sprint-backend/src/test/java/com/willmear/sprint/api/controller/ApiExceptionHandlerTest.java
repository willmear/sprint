...
class ApiExceptionHandlerTest {
    ...
    void shouldBuildIntegrationError() {
        ...
        try (ScopedLogLevel ignored = ScopedLogLevel.off(ApiExceptionHandler.class)) {
            response = handler.handleIntegration(new IntegrationException("Downstream failed."), request);
        }
    ...
    void shouldHideUnexpectedExceptionDetails() {
        ...
        ResponseEntity<ApiError> response;
        try (ScopedLogLevel ignored = ScopedLogLevel.off(ApiExceptionHandler.class)) {
            response = handler.handleUnexpected(new IllegalStateException("boom"), request);
        }

...
class AiBackedSprintReviewGenerationServiceTest {
    ...
    @Test
    void shouldAssembleReviewFromAiAndPlaceholderDerivedSections() {
        SprintContext context = context();
        SprintReviewGenerationInput input = input();
        AiBackedSprintReviewGenerationService service = new AiBackedSprintReviewGenerationService(
                aiFacade,
        ...
        when(placeholder.buildHighlights(context)).thenReturn(List.of(new SprintHighlight("Highlight", "Delivered", List.of("SPR-1"), "FEATURE")));
        when(placeholder.buildBlockers(context)).thenReturn(List.of());
